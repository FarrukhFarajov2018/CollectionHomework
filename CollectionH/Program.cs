...
namespace CollectionH
{
    ...
    class Action
    {
        ...
        public void sortbyMail(List<Debtor> debtors)
        {
            var rhitacom = debtors.FindAll(x => x.Email.Contains("rhyta.com"));

            foreach (var item in rhitacom)
            {
                Console.WriteLine($" {item} ");
            }
        }

        public void sortbyAge(List<Debtor> debtors)
        {
            var year2636 = debtors.FindAll(x => ((DateTime.Now - x.BirthDay).Days / 365) > 26 && ((DateTime.Now - x.BirthDay).Days / 365) < 36);

            foreach (var item in year2636)
            {
                Console.WriteLine($" {item} ");
            }
        }
        public void sortbyDebtAmount(List<Debtor> debtors)
        {
            var debt5000 = debtors.FindAll(x => x.Debt <= 5000);

            foreach (var item in debt5000)
            {
                Console.WriteLine($" {item} ");
            }
        }

        public void sortbySimbolsinNameTelNumber(List<Debtor> debtors)
        {
            var tel18simbols = debtors.FindAll(x => (x.FullName.Length > 18) || (x.Phone.Count(y => y == '7') >= 3));

            foreach (var item in tel18simbols)
            {
                Console.WriteLine($" {item} ");
            }
        }

        public void sortbyBirthMonth(List<Debtor> debtors)
        {

            var winterBirthday = debtors.FindAll(x => x.BirthDay.Month == 12 || x.BirthDay.Month >= 1 && x.BirthDay.Month <= 2);

            foreach (var item in winterBirthday)
            {
                Console.WriteLine($" {item} ");
            }
        }

        public void AverageDebtSort(List<Debtor> debtors)
        {
            ...
            int averageDebt = sum / debtors.Count;

            var averageDebtList = debtors.FindAll(x => x.Debt > averageDebt);

            Console.WriteLine(" Average Debt list");
            Console.WriteLine();
            Console.WriteLine("___________________________________");
            Console.WriteLine();

            foreach (var item in averageDebtList)
            {
                Console.WriteLine($" {item} ");
            }
            ...
            var sortbySurname = averageDebtList.OrderBy(x => x.FullName.Split(' ').ToList().Last());

            Console.WriteLine("Sort by Surname from Average Debt list");
            Console.WriteLine();

            foreach (var item in sortbySurname)
            {
                Console.WriteLine($" {item} ");
            }
            ...
            var averageDebtListAz = averageDebtList.OrderByDescending(x => x.Debt);

            Console.WriteLine("Sort by descending from Average Debt list");
            Console.WriteLine();

            foreach (var item in averageDebtListAz)
            {
                Console.WriteLine($" {item} ");
            }
        }

        public void SortBy8inTelNum(List<Debtor> debtors)
        {
            var numwithout8List = debtors.FindAll(x => !x.Phone.Contains("8"));

            foreach (var item in numwithout8List)
            {
                Console.WriteLine($" Surname - { item.FullName.Split(' ').ToList().Last()} , Age - {((DateTime.Now - item.BirthDay).Days / 365)} , Total Debt - {item.Debt} , Phone - {item.Phone}");
            }


        }

        string getname(string ad)
        {

            char[] myarr = new char[ad.Length];
            myarr = ad.ToCharArray();
            int found = 0;
        ...
        public void SortBy3sameLetterInName(List<Debtor> debtors)
        {
            var justNameList = debtors.Select(x => x.FullName);

            List<string> sorted3namelist = new List<string>();

            foreach (var item in justNameList)
            {
                if (getname(item).Length > 0)
                {
                    sorted3namelist.Add(item);
                }
            }


            List<Debtor> sorted3namelist1 = new List<Debtor>();
            for (int i = 0; i < sorted3namelist.Count(); i++)
            {
                sorted3namelist1.Add(debtors.Find(x => x.FullName == sorted3namelist[i]));
            }

            foreach (var item in sorted3namelist1)
            {
                Console.WriteLine(item);
            }
        ...
        public void SortbyGreatYear(List<Debtor> debtors)
        {
            DateTime GreatYear = new DateTime();

            int countF = 0;

            int found = 0;

            var Alldate = debtors.GroupBy(x => x.BirthDay.Year).ToList().OrderBy(x => x.Count());

            Console.WriteLine(Alldate.Last().Key);
        }

        public void Larger5Debters(List<Debtor> debtors)
        {
            var LargerDebters = debtors.OrderByDescending(x => x.Debt).ToList();


            for (int i = 0; i < 5; i++)
            {
                Console.WriteLine(LargerDebters[i]);
            }
        }

        public void totalDebt(List<Debtor> debtors)
        {
            var LargerDebters = debtors.OrderByDescending(x => x.Debt).ToList();
        ...
        public void WorldWar2(List<Debtor> debtors)
        {
            var WorldWar2Debtors = debtors.FindAll(x => x.BirthDay.Year >= 1941 && x.BirthDay.Year <= 1945);

            foreach (var item in WorldWar2Debtors)
            {
                Console.WriteLine(item);
            }
        }

        string findNorepeat(string justNum1)
        {
            
            string newJustname = justNum1.Replace("-", String.Empty);


            char[] myarr = new char[newJustname.Length];


            myarr = newJustname.ToCharArray();

            int found = 0;


            foreach (char item in newJustname)
            {
               

                for (int j = 0; j < newJustname.Length; j++)
                {
                    if (myarr[j] == item)
                    {
                        found++;
        ...
        public void Norepead(List<Debtor> debtors)
        {
            var justNum = debtors.Select(x => x.Phone);

           
            List<string> sortedNoRepead = new List<string>();


            foreach (var item in justNum)
            {
                if (findNorepeat(item).Length > 0)
                {
                    sortedNoRepead.Add(item);
                }
            }

            foreach (var item in sortedNoRepead)
            {
                var newDebtore = debtors.FindAll(x => x.Phone == item);

                foreach (var item1 in newDebtore)
                {
        ...
        public void PayTillBirthday(List<Debtor> debtors)
        {
            var Plist = debtors.FindAll(x => x.Debt <= ((DateTime.Now.Month - x.BirthDay.Month) * 500));
        ...
        public void NameWithSmile(List<Debtor> debtors)
        {
            var Slist = debtors.FindAll(x => (x.FullName.Contains('s') || x.FullName.Contains('S')) && 
            (x.FullName.Contains('m') || x.FullName.Contains('M')) && (x.FullName.Contains('i') || 
            x.FullName.Contains('I')) && (x.FullName.Contains('l') || x.FullName.Contains('L')) 
            && (x.FullName.Contains('e') || x.FullName.Contains('E')));
    ...
    static void Main(string[] args)
    {
        ...
        List<Debtor> debtors = new List<Debtor> {
             ...
             void switchFunc()
            {
                ...
                Console.WriteLine();
                Console.WriteLine("1. Sort by mail");
                Console.WriteLine("2. Sort by age-26-36");
                Console.WriteLine("3. Sort debt LESS than 5000");
                Console.WriteLine("4. Sort by name wiht 18 simbol OR two 7 in number");
                Console.WriteLine("5. Sort- Birthday in winter");
                Console.WriteLine("6. Sort- Debt more than average debt");
                Console.WriteLine("7. Sort- No 8 figure in telephone num");
                Console.WriteLine("8. Sort- 3 same letter in fullname");
                Console.WriteLine("9. Sort- Year with more debters");
                Console.WriteLine("10. Sort- 5 debters with more debt");
                Console.WriteLine("11. Sort- Total debt");
                Console.WriteLine("12. Sort- World War 2 debters");
                Console.WriteLine("13. Sort- Debters with no equal numbers in phone num");
                Console.WriteLine("14. Sort- Payment till birthday");
                Console.WriteLine("15. Sort- Name with SMILE");
                Console.WriteLine("16. Press 16 to leave program");
                ...
                int choice = Int32.Parse(Console.ReadLine());
                    ...
                    switch (choice)
                    {
                        case 1:
                            Console.Clear();
                            actobj.sortbyMail(debtors);

                            break;

                        case 2:
                            Console.Clear();
                            actobj.sortbyAge(debtors);

                            break;

                        case 3:
                            Console.Clear();
                            actobj.sortbyDebtAmount(debtors);

                            break;

                        case 4:
                            Console.Clear();
                            actobj.sortbySimbolsinNameTelNumber(debtors);

                            break;

                        case 5:
                            Console.Clear();
                            actobj.sortbyBirthMonth(debtors);

                            break;

                        case 6:
                            Console.Clear();
                            actobj.AverageDebtSort(debtors);

                            break;

                        case 7:
                            Console.Clear();
                            actobj.SortBy8inTelNum(debtors);

                            break;

                        case 8:
                            Console.Clear();
                            actobj.SortBy3sameLetterInName(debtors);

                            break;

                        case 9:
                            Console.Clear();
                            actobj.SortbyGreatYear(debtors);

                            break;

                        case 10:
                            Console.Clear();
                            actobj.Larger5Debters(debtors);

                            break;

                        case 11:
                            Console.Clear();
                            actobj.totalDebt(debtors);

                            break;

                        case 12:
                            Console.Clear();
                            actobj.WorldWar2(debtors);

                            break;

                        case 13:
                            Console.Clear();
                            actobj.Norepead(debtors);

                            break;

                        case 14:
                            Console.Clear();
                            actobj.PayTillBirthday(debtors);
                  ...
                  End: Console.WriteLine("Thanks for using program");
